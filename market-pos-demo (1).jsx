...
const CAT_OPTIONS = [
  {value:"chicken",label:"ไข่ไก่"},{value:"thin",label:"ไข่เปลือกบาง"},{value:"duck",label:"ไข่เป็ด"},
  {value:"century",label:"ไข่เยี่ยวม้า"},{value:"salty",label:"ไข่เค็ม"},{value:"fish",label:"ปลาทู"},{value:"custom",label:"สินค้าอื่น"},
];
const EMOJI_OPTIONS = ["🥚","🫧","🩶","🖤","🧂","🐟","📦","🐔","🦆","🛒","🍳","🐠","🌾","🥩","🧀"];
const COLOR_OPTIONS = [
  {color:"#F59E0B",bg:"#FFFBEB"},{color:"#84CC16",bg:"#F7FEE7"},{color:"#64748B",bg:"#F1F5F9"},
  {color:"#7C3AED",bg:"#FAF5FF"},{color:"#DC2626",bg:"#FFF1F2"},{color:"#0EA5E9",bg:"#F0F9FF"},
  {color:"#0369A1",bg:"#EFF6FF"},{color:"#059669",bg:"#ECFDF5"},{color:"#EC4899",bg:"#FDF2F8"},{color:"#374151",bg:"#F9FAFB"},
];
...
// ─────────────────────────────────────────────────────────────
const STORE_KEY = "market_demo_v3";
function loadStore() {
  try { const r = localStorage.getItem(STORE_KEY); if (r) return JSON.parse(r); } catch {}
  return { products: DEFAULT_PRODUCTS, sales: [], purchases: [] };
}
...
export default function App() {
  ...
  const products  = store.products.filter(p => p.active !== false).sort((a,b)=>(a.sort_order||99)-(b.sort_order||99)||a.name.localeCompare(b.name));
  ...
  const checkout = (method) => {
    ...
    const items = cart.map(i=>({
      id:i.id, name:i.name, emoji:i.emoji, unit:i.unit, size:i.size,
      qty:i.qty, price:i.price, cost:i.cost_unit*i.size,
      subtotal:i.price*i.qty, subtotal_cost:i.cost_unit*i.size*i.qty,
    }));
    const sale = {id:uid(), ts:new Date().toISOString(), method, total:cartRevenue, total_cost:cartCost, profit:cartProfit, items};
    persist({...store, sales:[sale,...sales]});
    setCart([]); setModal(null);
    showToast(`✅ ฿${fmt(cartRevenue)}  กำไร ฿${fmt(cartProfit)}`);
  };

  // ── Save Purchase ─────────────────────────────
  const savePurchase = ({productId, productName, qtyDisplay, unitLabel, unitsPerPack, totalCost}) => {
    const qtyUnit = qtyDisplay * unitsPerPack;
    const costPerUnit = parseFloat((totalCost / qtyUnit).toFixed(4));
    const row = {id:uid(), ts:new Date().toISOString(), product_id:productId, product_name:productName,
      qty_unit:qtyUnit, qty_display:qtyDisplay, unit_label:unitLabel, total_cost:totalCost, cost_per_unit:costPerUnit};
    const updatedProds = store.products.map(p => p.id===productId ? {...p, cost_unit:costPerUnit} : p);
    persist({...store, purchases:[row,...purchases], products:updatedProds});
    showToast(`✅ บันทึกแล้ว — ต้นทุน/หน่วย ฿${fmtFull(costPerUnit)}`);
    return true;
  };
  ...
  const saveProduct = (prod) => {
    let updatedProds;
    if (prod.id && store.products.find(p=>p.id===prod.id)) {
      updatedProds = store.products.map(p => p.id===prod.id ? {...p,...prod} : p);
    } else {
      const newP = {...prod, id:uid(), active:true, sort_order:99};
      updatedProds = [...store.products, newP];
    }
    persist({...store, products:updatedProds});
    showToast("✅ บันทึกสินค้าแล้ว");
    return true;
  };
  const deleteProduct = (id) => {
    persist({...store, products:store.products.map(p=>p.id===id?{...p,active:false}:p)});
  ...
  const groupedProds = {};
  products.forEach(p=>{ if(!groupedProds[p.cat])groupedProds[p.cat]=[]; groupedProds[p.cat].push(p); });

  // Voice for POS — แสดง confirmation ก่อน add
  const [voiceConfirm, setVoiceConfirm] = useState(null); // {product, unit, qty}
  const posVoice = useVoiceInput(products, ({ product, unitHint, qty, raw }) => {
    if (!product) {
      // ไม่เจอสินค้า — แสดงข้อความ
      return;
    }
    const units = getUnits(product);
    const matchedUnit = unitHint
      ? units.find(u => u.name.includes(unitHint.split(" ")[0])) || units[0]
      : units[0];
    const confirmedQty = qty || 1;
    setVoiceConfirm({ product, unit: matchedUnit, qty: confirmedQty });
  });

  // ─────────────────────────────────────────────
  return (
    <div style={{fontFamily:"'Noto Sans Thai',sans-serif",background:"#F1F5F9",minHeight:"100vh",display:"flex",flexDirection:"column",maxWidth:430,margin:"0 auto",position:"relative"}}>

      {/* Demo banner */}
      <div style={{background:"#1E293B",color:"#F59E0B",textAlign:"center",fontSize:12,fontWeight:700,padding:"6px 0",letterSpacing:0.5}}>
        🧪 DEMO MODE — ข้อมูลเก็บในเบราว์เซอร์นี้เท่านั้น
      </div>

      {/* HEADER */}
      <div style={{background:"linear-gradient(135deg,#F59E0B,#B45309)",padding:"14px 18px 10px",position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 14px rgba(180,83,9,0.28)"}}>
        ...
        <div style={{display:"flex",gap:2,marginTop:11,background:"rgba(0,0,0,0.18)",borderRadius:12,padding:3}}>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        ...
        {/* ════ BUY ════ */}
        {tab==="buy" && (
          <PurchaseTab products={products} purchases={purchases} onSave={savePurchase} showToast={showToast}/>
        )}
        ...
        {/* ════ MANAGE ════ */}
        {tab==="manage" && (
          <ManageTab products={products} onSave={saveProduct} onDelete={deleteProduct}/>
        )}
      </div>
      ...
      {modal?.type==="edit_product"&&(
        <BottomSheet onClose={()=>setModal(null)}>
          <ProductEditInner product={modal.product} onSave={(p)=>{saveProduct(p);setModal(null);}} onClose={()=>setModal(null)}/>
        </BottomSheet>
      )}
      ...
      {toast&&(
        <div style={{position:"fixed",top:92,left:"50%",transform:"translateX(-50%)",background:toast.color,color:"#fff",padding:"12px 20px",borderRadius:50,fontWeight:700,fontSize:14,zIndex:999,whiteSpace:"nowrap",maxWidth:"88vw",textAlign:"center",boxShadow:"0 4px 20px rgba(0,0,0,0.2)",animation:"toastIn 0.25s ease"}}>
          {toast.msg}
        </div>
      )}
      ...
      <style>{`
        ...
        *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;margin:0;padding:0;}
        ...
        @keyframes toastIn{from{opacity:0;top:72px}to{opacity:1;top:92px}}
